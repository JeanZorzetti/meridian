-- 0001_baseline — the schema as it stood before migrations existed.
--
-- Byte-for-byte the old db/schema.sql. That is the point: every statement here
-- is `if not exists`, so the production database — which already has all of it,
-- applied by hand via `npm run db:init` — adopts this migration without a single
-- object being recreated. Running it on an empty database builds the same schema
-- from scratch. Both paths converge here, and 0002 onward is the same file for
-- both.
--
-- Migrations from 0002 on are NOT expected to be idempotent: the runner records
-- what it applied and never applies it twice. Write them as plain forward steps.
--
-- Meridian — orçamento pessoal. Money is stored as integer cents.

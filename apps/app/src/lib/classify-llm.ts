...
// lives here.
//
// TEMPORARY DUPLICATE of apps/site/src/lib/classify-llm.ts, which is deleted
// when the site stops serving /api/* (rodada C in docs/HANDOFF.md 8.3). Until
// then only one of the two is reachable at a time — the site's, because this app
// is not deployed yet. Change both or neither.
...
// The Astro copy reads `import.meta.env` first, because Vite inlines the value
// there. Next has no such thing — server env is plain `process.env`.
